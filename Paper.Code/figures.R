rm(list=ls(all=TRUE))
library(xtable)
setwd("/Users/iad/Desktop/Study/3.Yale/Research/Wei Wei Project TDB /Biometrics_Revision/Biometrics_Revision_Data_hetero")
...
for(s in 2:6){
  ...
  axis(1, seq(3.5, 30, 6)[1:B], paste("", Indication_Name, sep=""))
  #mtext(paste(pmat0[s, ],"(", pmat1[s, ], ")", sep=""), side=1, cex=0.7, line=2.5, at=seq(3, 25, 5)[1:B], font=2)
  if(s==2){
    legend("topright", pch = 15, cex = 2, bty = "n",
           col = colors,
...
power_name_3 <- paste0("NI_power_3_","r_",r,".pdf")
pdf(power_name_3, width = 12, height = 4.5)  
par(mfrow = c(1, 3), oma = c(1,5,0.1,2), mar = c(4,2,2,2))
for (s in c(12, 13, 14)) {
  barplot(pwr ~ met + ind, data = bar_dat[bar_dat$sce==s, ], las = 1, 
          ylab = "", xlab = "", xaxt = "n", ylim = c(0, 1),
          beside = TRUE, cex.axis = 1.25, col = colors,
          main = paste("S", s-1, sep=""), cex.lab = 1.5)
  abline(h = 0.1, lty = 3, col = 2)
  axis(1, seq(3.5, 30, 6)[1:B], Indication_Name)
  if (s == 12) {
    legend("topright", pch = 15, cex = 2, bty = "n",
           col = colors, legend = c("REF","BMA","EXNEX","BMA-2D","EXNEX-2D"))
  }
}
dev.off()
...
for (s in 1:S){
  ...
  if (s >6 & s <= 11){
    AVP_0[s, 1] <- mean(ref_pow[s, 1:(s-6)])
    AVP_0[s, 2] <- mean(bma_pow[s, 1:(s-6)])
    AVP_0[s, 3] <- mean(bma2D_pow[s, 1:(s-6)])
  }
  if (s > 11){
    AVP_0[s, 1] <- mean(ref_pow[s, ])
    AVP_0[s, 2] <- mean(bma_pow[s, ])
...
for (s in 1:S){
  ...
  if (s >6 & s <= 11){
    AVP_m_2[s, 1] <- mean(ref_pow[s, 1:(s-6)])
    AVP_m_2[s, 2] <- mean(bma_pow[s, 1:(s-6)])
    AVP_m_2[s, 3] <- mean(bma2D_pow[s, 1:(s-6)])
  }
  if (s > 11){
    AVP_m_2[s, 1] <- mean(ref_pow[s, ])
    AVP_m_2[s, 2] <- mean(bma_pow[s, ])
...
for (s in 1:S){
  ...
  if (s >6 & s <= 11){
    AVP_2[s, 1] <- mean(ref_pow[s, 1:(s-6)])
    AVP_2[s, 2] <- mean(bma_pow[s, 1:(s-6)])
    AVP_2[s, 3] <- mean(bma2D_pow[s, 1:(s-6)])
  }
  if (s > 11){
    AVP_2[s, 1] <- mean(ref_pow[s, ])
    AVP_2[s, 2] <- mean(bma_pow[s, ])
...
bias_name_3 <- paste0("NI_bias_3_","r_", r, ".pdf")
pdf(bias_name_3, width = 12, height = 4.5)   
par(mfrow = c(1, 3), oma = c(1,5,0.1,2), mar = c(4,2,2,2))

for (s in 12:14) {
  boxplot(bias ~ met, data = box_dat[box_dat$sce == s,], las = 1,
          ylab = "", xlab = "", xaxt = "n", ylim = c(0, 0.4),
          cex.axis = 1.25, main = paste0("S", s-1), cex.lab = 1.5)
  axis(1, at = 1:7, labels = FALSE)
  labs <- c("REF","Pool","Test&Pool","BMA","EXNEX","BMA-2D","EXNEX-2D")
  text(x = 1:7, y = par("usr")[3] - 0.015, srt = 45, adj = 1,
       labels = labs, xpd = TRUE, cex = 1, font = 2)
}
dev.off()
################################################################################
